...
public class HttpUtils {
  ...
  // ??? вопрос нужно ли здесь обрезать путь, чтобы на клиенте снова его
  // воссоздавать?
  // возможно нужно ведь на клиенте мы запрашиваем без id устройства а внутренние
  // пути преобразуются уже с id под автозапросы от прокси при загрузки html. это
  // файлы стилей изображений и все внутренние ссылки, они также должны быть с id
  // устройстов чтобы подгружались сами
  public static String getPathFromRequest(HttpServletRequest request, String deviceId) {
    // Получаем оставшуюся часть пути
    // String remainingPath = (String)
  ...
  // перегруженный метод который создаёт заголовок после ответа, например после
  // редиректа 302
  public HashMap<String, String> buildHttpRequest(HashMap<String, String> buildHttpRequest, String setCookie)
      throws Exception {
  ...
  public HashMap<String, String> buildHttpRequest(HttpServletRequest request, String deviceId, String sessionId)
      throws Exception {
    ...
    /// !!!проверить как формируются гет параметры здесь!!!особенно множественные со
    /// знаком &
  ...
  public static byte[] buildMultipartFormData(String boundary, String fieldName, MultipartFile file)
      throws IOException {
