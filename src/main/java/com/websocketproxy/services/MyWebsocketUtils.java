...
public class MyWebsocketUtils {
  ...
  // Метод, возвращающий множество поддерживаемых методов сжатия, найденных в
  // заголовке
  ...
  // public static void sendChunkInputStream(WebSocketSession deviceSession,
  // String requestId, InputStream inputStream, boolean isGzip, int poss) throws
  // Exception {
  public static void sendChunkInputStream(WebSocketSession deviceSession, String requestId, InputStream inputStream,
      boolean isGzip) throws Exception {
    ...
    while ((bytesRead = inputStream.read(buffer)) != -1) {
      ...
      // isLast = (bytesRead < buffer.length); // Последняя часть, если прочитано
      // меньше буфера
      sendBinaryMessage(deviceSession, requestId, chunk, isLast, isGzip);
      MyLogger.logServer(
          "Sent " + chunk.length + " bytes for requestId: " + requestId + ", isLast: " + isLast + " isGzip=" + isGzip,
          true);
    }
    // если после всего цикла не было отправлено последнее сообщение, значит поток
    // был кратен размеру буфера. поэтому отправляем пустое последние сообщение. для
    // закрытия потока у клиента.
    if (!isLast) {
  ...
  // Отправка бинарного сообщения через WebSocket. первые байты, флаг сжатия и
  // requestId которые мы получим на стороне клиента всегда в не сжатом виде.
  public synchronized static void sendBinaryMessage(WebSocketSession session, String requestId, byte[] data,
      boolean isLast, boolean isGzip) throws Exception {
    // synchronized (session) {
    if (session.isOpen()) {
      ...
      if (!isLast) {
        if (!isGzip)
          MyLogger.logServByteToString(data);
        else
          MyLogger.logSrvDecodeUnGzip(data);
      }
  ...
  public void sendMultipartFormDataStream(WebSocketSession deviceSession, String requestId, String boundary,
      String fieldName, MultipartFile file) throws Exception {
  ...
  // byte[] compressedData =
  // compress(requestBody.getBytes(StandardCharsets.UTF_8));
  // sendBinaryMessage(deviceSession, requestId, compressedData);
  // }

  // !!! нужно разжимать все стандартные алгоритмы Content-Encoding, включая
